...
const IconBtn: React.FC<IconBtnProps> = ({ icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="text-on-surface-variant hover:text-primary transition p-1 rounded-full hover:bg-surface-container-high flex items-center justify-center"
    >
  ...
  return (
    <Fragment>
      <header className="grid grid-cols-[320px_1fr] border-b border-outline-variant/30 bg-surface/80 backdrop-blur-md">
        ...
        <div className=" flex gap-3 px-6 py-4 border-r border-outline-variant/30   justify-start">
          <Menu
            className="md:hidden cursor-pointer text-on-surface-variant hover:text-primary transition"
            onClick={() => setIsDrawerOpen(true)}
          />
          <div className="flex flex-col">
            <h1 className="text-headline-md transition-colors font-bold tracking-tight leading-none">
              Quran Mazid
            </h1>
            <p className=" md:flex  hidden text-[10px] text-on-surface-variant/50 font-medium uppercase mt-1.5 tracking-wider">
        ...
        <div className="px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className=" md:flex hidden  items-center gap-2 bg-primary hover:bg-primary/90 text-on-primary px-5 py-2.5 rounded-full text-xs font-bold transition-all shadow-lg shadow-primary/20">
